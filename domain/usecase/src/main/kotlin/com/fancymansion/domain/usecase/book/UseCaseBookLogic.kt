...
class UseCaseBookLogic @Inject constructor(
    ...
    suspend fun incrementActionCount(episodeRef: EpisodeRef, pageId: Long) =
        incrementActionCount(episodeRef, ActionIdModel(pageId = pageId))
    ...
    private suspend fun checkConditions(
        ...
        for (condition in conditions) {
            ...
            logicalOp = condition.conditionRule.logicalOp
    ...
    private suspend fun checkCondition(
        ...
        return condition.conditionRule.let { rule ->
            val selfActionCount = bookLocalRepository.getActionCount(episodeRef, actionId = rule.selfActionId)?:0

            val targetCount : Int = when(rule){
                is ConditionRuleModel.CountConditionRuleModel -> {
                    rule.count
                }

                is ConditionRuleModel.TargetConditionRuleModel -> {
                    bookLocalRepository.getActionCount(episodeRef, actionId = rule.targetActionId)
                }
            }?:0

            rule.relationOp.check(selfActionCount, targetCount)
        }
    }
}